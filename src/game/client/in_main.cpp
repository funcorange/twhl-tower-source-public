...
bool	g_bIsForwardSpeedOverriden;
bool	g_bIsSideSpeedOverriden;
bool	g_bIsBackSpeedOverriden;
float	g_flForwardSpeedOverride;
float	g_flSideSpeedOverride;
float	g_flBackSpeedOverride;
...
float GetSideSpeed()
{
	if (g_bIsSideSpeedOverriden)
		return g_flSideSpeedOverride;
...
float GetBackSpeed()
{
	if (g_bIsBackSpeedOverriden)
		return g_flBackSpeedOverride;

	return cl_backspeed.GetFloat();
}


float GetForwardSpeed()
{
	if (g_bIsForwardSpeedOverriden)
		return g_flForwardSpeedOverride;
...
void CInput::ComputeSideMove( CUserCmd *cmd )
{
	...
	if ( CAM_IsThirdPerson() && thirdperson_screenspace.GetInt() )
	{
		...
		cmd->sidemove += GetSideSpeed() * movement;
	...
	if ( in_strafe.state & 1 )
	{
		cmd->sidemove += GetSideSpeed() * KeyState (&in_right);
		cmd->sidemove -= GetSideSpeed() * KeyState (&in_left);
	}

	// Otherwise, check strafe keys
	cmd->sidemove += GetSideSpeed() * KeyState (&in_moveright);
	cmd->sidemove -= GetSideSpeed() * KeyState (&in_moveleft);
}
...
void CInput::ComputeForwardMove( CUserCmd *cmd )
{
	...
	if ( CAM_IsThirdPerson() && thirdperson_platformer.GetInt() )
	{
		...
		cmd->forwardmove += GetForwardSpeed() * movement;
	...
	if ( !(in_klook.state & 1 ) )
	{	
		cmd->forwardmove += GetForwardSpeed() * KeyState (&in_forward);
		cmd->forwardmove -= GetBackSpeed() * KeyState(&in_back);
	}	
}
