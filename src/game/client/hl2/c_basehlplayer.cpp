...
void C_BaseHLPlayer::PerformClientSideObstacleAvoidance( float flFrameTime, CUserCmd *pCmd )
{
	...
	if ( pCmd->forwardmove > fabs( GetForwardSpeed() ) )
	{
		flForwardScale = fabs( GetForwardSpeed() ) / pCmd->forwardmove;
	}
	else if ( pCmd->forwardmove < -fabs( GetBackSpeed() ) )
	{
		flForwardScale = fabs( GetBackSpeed() ) / fabs( pCmd->forwardmove );
	}
	
	float flSideScale = 1.0f;
	if ( fabs( pCmd->sidemove ) > fabs( GetSideSpeed() ) )
	{
		flSideScale = fabs( GetSideSpeed() ) / fabs(pCmd->sidemove);
	}
...
void C_BaseHLPlayer::PerformClientSideNPCSpeedModifiers( float flFrameTime, CUserCmd *pCmd )
{
	...
	{
		if ( m_flSpeedMod != GetForwardSpeed() )
		{
			float flDeltaTime = (m_flSpeedModTime - gpGlobals->curtime);
			m_flSpeedMod = RemapValClamped( flDeltaTime, cl_npc_speedmod_outtime.GetFloat(), 0, m_flExitSpeedMod, GetForwardSpeed() );
		}
	}
	else
	{
		...
		if ( pNPC )
		{
			...
			else 
			{
				...
				{
					float flDeltaTime = (m_flSpeedModTime - gpGlobals->curtime);
					m_flSpeedMod = RemapValClamped( flDeltaTime, cl_npc_speedmod_intime.GetFloat(), 0, GetForwardSpeed(), pNPC->GetSpeedModifySpeed() );
				}
			}
		}
	}
