...
void CHudCredits::DrawLogo( void )
{
	...
	if (m_iLogoImageID != -1)
	{
		float flImageScale = m_flLogoImageScale * (GetTall() / 900.0F);
		int iImageWide = 0;
		int iImageTall = 0;
		surface()->DrawGetTextureSize(m_iLogoImageID, iImageWide, iImageTall);
		iImageWide = iImageWide * flImageScale;
		iImageTall = iImageTall * flImageScale;
		Color cColor = Color(255, 255, 255, m_Alpha);
		if (m_cLogoColor.a() > 0)
		{
			cColor = m_cLogoColor;
		}
		surface()->DrawSetColor(cColor);
		surface()->DrawSetTexture(m_iLogoImageID);
...
#ifdef TWHLTOWER
	if (m_szLogoFont[0] == '$' && V_strncmp(m_szLogoFont, "Image", 5))
	{
		if (m_szLogoFont[6] == ';')
		{
			CUtlStringList outStrings;
			V_SplitString(m_szLogoFont, ";", outStrings);
			FOR_EACH_VEC(outStrings, i)
			{
				switch (i)
				{
					// Get scale
					case 1:
						m_flLogoImageScale = atof(outStrings[i]);
						break;
					...
					case 2:
						...
						m_cLogoColor = Color(tmp[0], tmp[1], tmp[2], tmp[3]);
						break;
				}
			}
			outStrings.PurgeAndDeleteElements();
		}
		m_iLogoImageID = GetOrAllocateImageID(m_szLogo);
	}
#endif // TWHLTOWER
	SetActive( true );
}
