...
#include "twhltower/c_twhltower_player.h"
...
bool	g_bIsMousePitchOverriden;
float	g_flMousePitchOverride;
...
//------------------------------------------------------------------------------
// Purpose: Returns vertical mouse scaling
//------------------------------------------------------------------------------
float GetMousePitchScale()
{
	if (g_bIsMousePitchOverriden)
		return (m_pitch->GetFloat() < 0) ? -g_flMousePitchOverride : g_flMousePitchOverride;
...
#endif //#ifdef PORTAL
		{
			if ( CAM_IsThirdPerson() && thirdperson_platformer.GetInt() )
			{
				if ( mouse_y )
				{
					...
					// use the mouse to orbit the camera around the player, and update the idealAngle
					vTempOffset[ PITCH ] += GetMousePitchScale() * mouse_y;
			...
			{
				viewangles[PITCH] += GetMousePitchScale() * mouse_y;
			}
