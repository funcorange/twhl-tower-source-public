...
void CAchievementNotificationPanel::PerformLayout( void )
{
	...
	m_pPanelBackground->SetBgColor(Color(0, 0, 0, 150));
}
...
{
	if (!m_bHiding && (m_flHideTime > 0) && (gpGlobals->curtime >= m_flHideTime - 0.5f))
	{
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence("HideAchievementNotificationPanel");
		m_bHiding = true;
	}
...
void CAchievementNotificationPanel::ShowNextNotification()
{
	...
	g_pClientMode->GetViewportAnimationController()->StartAnimationSequence("ShowAchievementNotificationPanel");
...
CON_COMMAND_F( achievement_notification_test, "Test the hud notification UI", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY )
{
	...
	if ( pPanel )
	{		
		pPanel->AddNotification( "TOWER_PLACEHOLDER", L"Achievement Progress", ( 0 == ( iCount % 2 ) ? L"Test Notification Message A (1/10)" :
			L"Test Message A" ) );
	}
#endif
	...
	if (event)
	{
		const char szName[] = "TOWER_PLACEHOLDER";
		event->SetString("achievement_name", szName);
		event->SetInt("cur_val", (iCount%3) + 1);
		event->SetInt("max_val", 3);
		gameeventmanager->FireEvent(event);
	}
#endif
