...
#ifdef TWHLTOWER
#include "twhltower/twhltower_hud_saverestore.h"
#include "twhltower/twhltower_options.h"
#endif
...
// CTWHLTowerClient
//=============================================================================
class CTWHLTowerClient : public CHLClient
{
public:
	using ThisClass = CTWHLTowerClient;
	using BaseClass = CHLClient;
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CTWHLTowerClient::CTWHLTowerClient() :
	BaseClass()
{
}
...
//-----------------------------------------------------------------------------
int CTWHLTowerClient::Init(
	CreateInterfaceFn appSystemFactory,
	CreateInterfaceFn physicsFactory,
	CGlobalVarsBase* pGlobals
)
{
	const int result = BaseClass::Init(
		appSystemFactory,
		physicsFactory,
		pGlobals
	);
	...
	g_pGameSaveRestoreBlockSet->AddBlockHandler(
		GetTWHLTowerHudSaveRestoreBlockHandler()
	);

#if 0
	IGameSystem::Add(TwhlTowerOptions());
#endif
...
//-----------------------------------------------------------------------------
// Purpose: Shutdown client
//-----------------------------------------------------------------------------
void CTWHLTowerClient::Shutdown()
{
