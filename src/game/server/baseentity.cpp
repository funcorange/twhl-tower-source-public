...
#ifdef TWHLTOWER
	m_colGlowColorOverride = {0, 0, 0, 0};
#endif
}
...
#ifdef TWHLTOWER
	SendOnTakeDamageGameEvent(info);
#endif
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseEntity::SendOnTakeDamageGameEvent(const CTakeDamageInfo& info)
{
	IGameEvent* pEvent = gameeventmanager->CreateEvent("entity_damaged");
	if (!pEvent)
		return;

	pEvent->SetInt("entindex_damaged", entindex());
	if (info.GetAttacker())
		pEvent->SetInt("entindex_attacker", info.GetAttacker()->entindex());
	if (info.GetInflictor())
		pEvent->SetInt("entindex_inflictor", info.GetInflictor()->entindex());
	pEvent->SetFloat("damage", info.GetDamage());
	pEvent->SetInt("damagebits", info.GetDamageType());
	pEvent->SetString("classname_damaged", GetClassname());
	pEvent->SetInt("health", GetHealth());
...
#ifdef TWHLTOWER
	DEFINE_KEYFIELD(m_colGlowColorOverride, FIELD_COLOR32, "glowcoloroverride"),
#endif
...
void CBaseEntity::PhysicsTouchTriggers( const Vector *pPrevAbsOrigin )
{
	...
	if ( pEdict && !IsWorld() )
	{
		...
		{
#ifdef TWHLTOWER
			// Hack for allowing weapons and items to collide with triggers
			if (GetCollisionGroup() == COLLISION_GROUP_WEAPON)
			{
