...
#ifdef TWHLTOWER
	DEFINE_KEYFIELD(m_iszGlowProxyName, FIELD_STRING, "glowproxy"),
#endif
...
//-----------------------------------------------------------------------------
// Purpose: Get the glow model entity for this brush
//-----------------------------------------------------------------------------
CBaseEntity* CBaseToggle::GetGlowProxy()
{
	// Don't glow if there's no glow entity
	if (m_iszGlowProxyName == NULL_STRING)
		return NULL;

	// Get glow entity
	return gEntList.FindEntityByNameFast(NULL, m_iszGlowProxyName);
}
