...
#ifdef TWHLTOWER
	DEFINE_KEYFIELD(m_bEnableFarGlow, FIELD_BOOLEAN, "enablefarglow"),
	DEFINE_INPUTFUNC(FIELD_VOID, "EnableFarGlow", InputEnableFarGlow),
	DEFINE_INPUTFUNC(FIELD_VOID, "DisableFarGlow", InputDisableFarGlow),
	DEFINE_INPUTFUNC(FIELD_COLOR32, "SetGlowColorOverride", InputSetGlowColorOverride),
#endif
...
#ifdef TWHLTOWER
//-----------------------------------------------------------------------------
// Purpose: Determine whether this prop should glow from far away
//-----------------------------------------------------------------------------
bool CBreakableProp::UseFarItemGlow()
{
	if (m_bEnableFarGlow)
		return true;

	if (m_explodeDamage > 0.0F || m_explodeRadius > 0.0F || HasInteraction(PROPINTER_PHYSGUN_BREAK_EXPLODE) || HasInteraction(PROPINTER_FIRE_IGNITE_HALFHEALTH))
		return true;

	return BaseClass::UseFarItemGlow();
}


//-----------------------------------------------------------------------------
// Purpose: Determine the glow color for this prop
//-----------------------------------------------------------------------------
color32 CBreakableProp::GetGlowColorOverride()
{
	if (BaseClass::GetGlowColorOverride() != color32{0, 0, 0, 0})
		return BaseClass::GetGlowColorOverride();

	if (m_explodeDamage > 0.0F || m_explodeRadius > 0.0F || HasInteraction(PROPINTER_PHYSGUN_BREAK_EXPLODE) || HasInteraction(PROPINTER_FIRE_IGNITE_HALFHEALTH))
		return color32{255, 64, 0, 255};
...
//----------------------------------------------------------------------------
// Purpose: Enable far glow for this prop
//----------------------------------------------------------------------------
void CBreakableProp::InputEnableFarGlow(inputdata_t& inputdata)
{
	m_bEnableFarGlow = true;
}


//----------------------------------------------------------------------------
// Purpose: Disable far glow for this prop
//----------------------------------------------------------------------------
void CBreakableProp::InputDisableFarGlow(inputdata_t& inputdata)
{
	m_bEnableFarGlow = false;
}


//----------------------------------------------------------------------------
// Purpose: Sets the far glow color for this prop
//----------------------------------------------------------------------------
void CBreakableProp::InputSetGlowColorOverride(inputdata_t& inputdata)
{
	SetGlowColorOverride(inputdata.value.Color32());
}
...
#ifdef TWHLTOWER
	bool UseNearItemGlow() OVERRIDE;
	bool UseFarItemGlow() OVERRIDE;
	CBaseEntity* GetGlowProxy() OVERRIDE;
#endif // TWHLTOWER
...
#ifdef TWHLTOWER
	string_t	m_iszGlowProxyOverride;
#endif
};
...
#ifdef TWHLTOWER
	DEFINE_KEYFIELD( m_iszGlowProxyOverride, FIELD_STRING, "GlowProxy"),
#endif
...
#ifdef TWHLTOWER
bool CInteractableProp::UseNearItemGlow()
{
	if (!UseFarItemGlow())
		return false;
...
bool CInteractableProp::UseFarItemGlow()
{
	if (m_flCooldownTime > gpGlobals->curtime)
		return false;
...
CBaseEntity* CInteractableProp::GetGlowProxy()
{
	if (m_iszGlowProxyOverride == NULL_STRING)
		return BaseClass::GetGlowProxy();

	CBaseEntity* pProxy = gEntList.FindEntityByName(
		NULL,
		m_iszGlowProxyOverride
	);

	if (!pProxy && !pProxy->GetBaseAnimating())
		return BaseClass::GetGlowProxy();
