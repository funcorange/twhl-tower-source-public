...
//-----------------------------------------------------------------------------
// Purpose: Determine whether to use glow effect
//-----------------------------------------------------------------------------
bool CBaseDoor::UseNearItemGlow()
{
	// Only show glow effect on usable doors
	if (!HasSpawnFlags(SF_DOOR_PUSE))
		return false;

	// Don't show glow on locked doors
	if (m_bLocked)
		return false;

	// Don't show glow on doors that cannot be opened
	if (HasSpawnFlags(SF_DOOR_NEW_USE_RULES))
	{
		if (!(m_toggle_state == TS_AT_BOTTOM || m_toggle_state == TS_GOING_DOWN) && !(HasSpawnFlags(SF_DOOR_NO_AUTO_RETURN) && (m_toggle_state == TS_AT_TOP || m_toggle_state == TS_GOING_UP)))
			return false;
	...
	{
		// Don't show glow on moving doors
		if (m_toggle_state == TS_GOING_UP || m_toggle_state == TS_GOING_DOWN)
			return false;

		// Don't show glow on doors that are waiting to close
		if (!HasSpawnFlags(SF_DOOR_NO_AUTO_RETURN) && m_toggle_state == TS_AT_TOP)
			return false;
	}
