...
ConVar	item_glow_critical_health_ratio("item_glow_critical_health_ratio", "0.19", FCVAR_CHEAT, "Use critical color for health item glows if player health is less than this ratio of max health.");

namespace
{
const color32 g_colHealthNormalGlowColor{0, 192, 255, 255};
const color32 g_colHealthCriticalGlowColor{255, 0, 0, 255};
}

//-----------------------------------------------------------------------------
// Purpose: Get the item glow outline color for health items
//-----------------------------------------------------------------------------
color32 GetHealthItemGlowColor()
{
	CBasePlayer* pPlayer = UTIL_GetLocalPlayer();
	if (!pPlayer || (static_cast<float>(pPlayer->GetHealth()) / static_cast<float>(pPlayer->GetMaxHealth()) > item_glow_critical_health_ratio.GetFloat()))
		return g_colHealthNormalGlowColor;

	return g_colHealthCriticalGlowColor;
}
...
#ifdef TWHLTOWER
	inline color32 GetGlowColorOverride() OVERRIDE { return GetHealthItemGlowColor(); }
#endif
...
#ifdef TWHLTOWER
	bool	UseNearItemGlow() OVERRIDE { return UseFarItemGlow(); }
	bool	UseFarItemGlow() OVERRIDE { return (m_iJuice > 0); }
	inline color32 GetGlowColorOverride() OVERRIDE { return GetHealthItemGlowColor(); }
#endif
