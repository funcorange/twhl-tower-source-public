...
#ifdef TWHLTOWER
//-----------------------------------------------------------------------------
// Purpose: Determine whether this button should glow
//-----------------------------------------------------------------------------
bool CBaseButton::UseNearItemGlow()
{
	// Only show glow effect on usable buttons
	if (!HasSpawnFlags(SF_BUTTON_USE_ACTIVATES))
		return false;

	// Don't show glow on moving buttons
	if (m_toggle_state == TS_GOING_DOWN || m_toggle_state == TS_GOING_UP)
		return false;

	// Don't show glow on buttons that haven't reset yet
	if (m_toggle_state == TS_AT_TOP && !HasSpawnFlags(SF_BUTTON_TOGGLE))
		return false;

	// Don't show glow on locked buttons
	if (m_bLocked)
		return false;

	return BaseClass::UseNearItemGlow();
}
#endif
...
#ifdef TWHLTOWER
bool CMomentaryRotButton::UseNearItemGlow()
{
	if (m_bDisabled)
		return false;
